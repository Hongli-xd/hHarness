...
class AnthropicLLMOptions(BindingOptions):
    ...
    temperature: float = DEFAULT_TEMPERATURE  # Controls randomness (0.0 to 1.0)
    max_tokens: int = 8192  # Maximum tokens to generate
    stop_sequences: List[str] = field(default_factory=list)  # Stop sequences
    top_p: float = None  # Nucleus sampling parameter (Anthropic recommends not setting this)
    top_k: int = None  # Top-k sampling parameter
    thinking: dict = None  # Extended thinking configuration
    metadata: dict = None  # Optional metadata to include with requests

    _help: ClassVar[dict[str, str]] = {
        "temperature": "Controls randomness (0.0-1.0, higher = more creative, default: 1.0)",
        "max_tokens": "Maximum tokens to generate (default: 8192)",
        "stop_sequences": "Stop sequences (list of strings)",
        "top_p": "Nucleus sampling parameter (Anthropic recommends not setting this)",
        "top_k": "Limits sampling to the top K tokens (Anthropic recommends not setting this)",
        "thinking": "Extended thinking configuration (JSON dict, e.g., '{\"thinking_budget\": 1024}')",
        "metadata": "Optional metadata (JSON dict, e.g., '{\"user_id\": \"123\"}')",
    }
