...
async def anthropic_complete_if_cache(
    ...
    **kwargs: Any,
    ...
    try:
        stream = kwargs.pop("stream", False)  # Default to False for non-streaming
        create_params = {"model": model, "messages": messages, "stream": stream, **kwargs}
        if system_prompt:
    ...
    if not stream:
        # Non-streaming: response is a Message object directly
        try:
            # Extract text from the Message content
            if hasattr(response, "content"):
                # Message object - access content blocks
                content_parts = []
                for block in response.content:
                    if hasattr(block, "text") and block.text:
                        content_parts.append(block.text)
                full_content = "".join(content_parts)
                if r"\u" in full_content:
                    full_content = safe_unicode_decode(full_content.encode("utf-8"))
                return full_content
            return str(response)
        except Exception as e:
