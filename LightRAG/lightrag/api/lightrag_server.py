...
class LLMConfigCache:
    ...
    def __init__(self, args):
        ...
        # Only initialize and log Anthropic LLM options when using Anthropic LLM binding
        if args.llm_binding == "anthropic":
            try:
                from lightrag.llm.binding_options import AnthropicLLMOptions

                self.anthropic_llm_options = AnthropicLLMOptions.options_dict(args)
                logger.info(f"Anthropic LLM Options: {self.anthropic_llm_options}")
            except ImportError:
                logger.warning(
                    "AnthropicLLMOptions not available, using default configuration"
                )
                self.anthropic_llm_options = {}
...
def create_app(args):
    ...
    def create_optimized_anthropic_llm_func(
        config_cache: LLMConfigCache, args, llm_timeout: int
    ):
        """Create optimized Anthropic LLM function with cached configuration"""
        ...
        async def optimized_anthropic_model_complete(
            ...
            **kwargs,
            ...
            # Use pre-processed configuration to avoid repeated parsing
            kwargs["timeout"] = llm_timeout
            if config_cache.anthropic_llm_options:
                # Map Anthropic options to the expected parameter names
                anthropic_opts = config_cache.anthropic_llm_options
                if "max_tokens" in anthropic_opts:
                    kwargs["max_tokens"] = anthropic_opts["max_tokens"]
                if "temperature" in anthropic_opts:
                    kwargs["temperature"] = anthropic_opts["temperature"]
                if "stop_sequences" in anthropic_opts:
                    kwargs["stop"] = anthropic_opts["stop_sequences"]
                if "thinking" in anthropic_opts:
                    kwargs["thinking"] = anthropic_opts["thinking"]
                if "metadata" in anthropic_opts:
                    kwargs["metadata"] = anthropic_opts["metadata"]
    ...
    def create_llm_model_func(binding: str):
        ...
        try:
            if binding == "lollms":
                ...
                return create_optimized_anthropic_llm_func(
                    config_cache, args, llm_timeout
                )
